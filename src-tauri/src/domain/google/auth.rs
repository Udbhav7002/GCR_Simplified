...
use crate::core::commands::{CancellationToken, LoginCancelFlag};
...
pub async fn start_google_login(
    _app: AppHandle,
    pool: State<'_, DbPool>,
    cancel_flag: State<'_, LoginCancelFlag>,
    ...
    if !resp.status().is_success() {
        return Err(super::handle_http_error(resp, "Token exchange").await);
    }
    ...
    if !profile_resp.status().is_success() {
        return Err(super::handle_http_error(profile_resp, "Get user profile").await);
    }
...
#[tauri::command]
pub async fn cancel_google_login(cancel_flag: State<'_, LoginCancelFlag>) -> Result<(), String> {
...
pub async fn google_logout(pool: State<'_, DbPool>) -> Result<(), String> {
    ...
    security::delete_secret(&pool, "google_access_token").ok();
    security::delete_secret(&pool, "google_refresh_token").ok();
