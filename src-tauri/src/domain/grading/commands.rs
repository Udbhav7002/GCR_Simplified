...
#[tauri::command]
pub async fn email_grades_to_students(
    assignment_id: String,
    state: tauri::State<'_, crate::db::DbPool>,
) -> Result<usize, String> {
    let view = get_gradebook(state.clone(), assignment_id).await?;
    
    // total possible marks can be derived from rubric max_marks
    let max_score: f64 = view.rubric.iter().map(|r| r.max_marks).sum();
    
    let mut sent_count = 0;
    
    for row in view.rows {
        // Find if any grade is approved (or we could just email if grading_status == "graded"?)
        // Let's only email if it has an approved grade
        if row.grades.iter().any(|g| g.approved) {
            if let Some(email) = row.student_email {
                if !email.is_empty() {
                    let total = row.grades.iter().filter_map(|g| g.score).sum::<f64>();
                    let grade_summary = format!("{total} / {max_score}");
                    
                    let mut feedback_str = String::new();
                    if let Some(fb) = row.ai_feedback {
                        feedback_str.push_str(&fb);
                    }
                    // Include any teacher override feedback?
                    for g in &row.grades {
                        if g.graded_by == "teacher" {
                            if let Some(tfb) = &g.feedback {
                                if !feedback_str.is_empty() {
                                    feedback_str.push_str("\n\n");
                                }
                                feedback_str.push_str("Teacher Feedback:\n");
                                feedback_str.push_str(tfb);
                            }
                        }
                    }
                    
                    let feedback_opt = if feedback_str.is_empty() { None } else { Some(feedback_str.as_str()) };
                    
                    // Call gmail.rs function
                    match crate::domain::google::gmail::send_grade_email(
                        &state,
                        &email,
                        ...
                        Ok(_) => { sent_count += 1; },
                        Err(e) => { log::error!("Failed to email grade to {}: {}", email, e); }
                    }
                }
            }
        }
