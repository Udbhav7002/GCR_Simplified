...
pub fn load_settings(pool: &DbPool) -> Result<AppSettings, String> {
    Ok(AppSettings {
        ...
        gemini_model: db_get_setting(pool, "gemini_model")?
            .unwrap_or_else(|| DEFAULT_GEMINI_MODEL.to_string()),
        default_fingerprint_threshold: db_get_setting(pool, "default_fingerprint_threshold")?
            .and_then(|v| v.parse().ok())
            .unwrap_or(DEFAULT_FINGERPRINT_THRESHOLD),
        default_semantic_threshold: db_get_setting(pool, "default_semantic_threshold")?
            .and_then(|v| v.parse().ok())
            .unwrap_or(DEFAULT_SEMANTIC_THRESHOLD),
        theme: db_get_setting(pool, "theme")?.unwrap_or_else(|| DEFAULT_THEME.to_string()),
        download_concurrency: db_get_setting(pool, "download_concurrency")?
            .and_then(|v| v.parse().ok())
            .unwrap_or(DEFAULT_DOWNLOAD_CONCURRENCY),
        extraction_concurrency: db_get_setting(pool, "extraction_concurrency")?
            .and_then(|v| v.parse().ok())
...
pub fn save_settings(pool: State<'_, DbPool>, settings: AppSettings) -> Result<(), String> {
    log::info!("Received save_settings: {:?}", settings);
    ...
    match &settings.gemini_api_key {
        ...
        Some(_) => {
            security::delete_secret(&pool, "gemini_api_key").ok();
        }
        None => {}
    }
